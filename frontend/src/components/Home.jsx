...
function Home() {
    return (
        <div className="relative h-screen w-full flex flex-col items-center justify-center text-gray-100 overflow-hidden">
            ...
            {/* Enhanced Red Glow */}
            <motion.div 
                className="absolute w-[400px] h-[400px] bg-red-600 opacity-30 rounded-full blur-[180px]"
                initial={{ x: "-50%", y: "-50%" }}
                animate={{ x: ["-30%", "25%", "-30%"], y: ["-20%", "15%", "-20%"] }}
                transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
            ></motion.div>

            {/* More Visible Navy Blue Glow */}
            <motion.div 
                className="absolute bottom-10 right-10 w-[350px] h-[350px] bg-blue-700 opacity-25 rounded-full blur-[170px]"
                initial={{ x: "50%", y: "50%" }}
                animate={{ x: ["25%", "-30%", "25%"], y: ["15%", "-20%", "15%"] }}
                transition={{ duration: 14, repeat: Infinity, ease: "easeInOut" }}
            ></motion.div>

            {/* New Purple Glow for a Galactic Effect */}
            <motion.div 
                className="absolute top-16 left-1/2 w-[300px] h-[300px] bg-purple-600 opacity-20 rounded-full blur-[160px]"
                initial={{ x: "-40%", y: "-30%" }}
